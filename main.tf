...
resource "aws_apigateway_api" "hello-world-func_resource" {
  name          = "app-api"     # Provide a name for your API
  protocol_type = "HTTP"                # You can choose HTTP, WebSocket, etc.
}

resource "aws_apigateway_route" "hello-world-route" {
  api_id    = aws_apigateway_api.hello-world-api.id
  route_key = "GET /hello-world"
  target    = "integrations/your-lambda-integration-id"
}
# Create an HTTP API route (e.g., /hello-world)
resource "aws_apigateway_resource" "hello-world-func_resource" {
  api_id   = aws_apigateway_api.app_api.id
  parent_id = aws_apigateway_api.hello-world-func_resource.api_endpoint
...
resource "aws_apigateway_deployment" "api_deployment" {
  depends_on  = [aws_apigateway_route.hello-world-func_route]
  api_id      = aws_apigateway_api.app_api.id
}
...
output "api_url" {
  value = aws_apigateway_api.app_api.api_endpoint
}
